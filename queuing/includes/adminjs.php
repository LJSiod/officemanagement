  ...
  $(document).ready(function () {
    ...
    $(document).on('contextmenu', '#queuetable table tbody tr', function (e) {
      ...
      var menu = $('<div class="dropdown-menu small removedrop" id="actiondropdown" style="display:block; position:absolute; z-index:1000;">'
        + (served == '<?php echo $_SESSION['userid']; ?>' ? '<a class="dropdown-item small" href="#" id="receive"><i class="fa fa-check-circle text-success" aria-hidden="true"></i> <span>Accomplished: Received</span></a>' : '<a class="dropdown-item small disabled" href="#" id="receive"><i class="fa fa-lock text-secondary dont" aria-hidden="true"></i> <span class="ml-2">Restricted</span></a>')
        + (served == '<?php echo $_SESSION['userid']; ?>' ? '<a class="dropdown-item small" href="#" id="decline"><i class="fa fa-times-circle text-danger" aria-hidden="true"></i> Accomplished: Declined</a>' : '')
        + (served == '<?php echo $_SESSION['userid']; ?>' ? '<a class="dropdown-item small" href="preview.php?id=' + id + '" id="preview"><i class="fa fa-eye text-info" aria-hidden="true"></i> Preview</a>' : '')
        + (served == '<?php echo $_SESSION['userid']; ?>' ? '<a class="dropdown-item small" href="#" id="return"><i class="fa fa-undo text-primary" aria-hidden="true"></i> Return to Queue</a>' : '')
        + '</div>').appendTo('body');
