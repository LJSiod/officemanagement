    ...
    $(document).ready(function () {
        ...
        $(document).on('contextmenu', '#queuetable table tbody tr', function (e) {
            ...
            var menu = $('<div class="dropdown-menu small removedrop" style="display:block; position:absolute; z-index:1000;">'
                + (branchid == '<?php echo $_SESSION['branchid']; ?>' ? '<a class="dropdown-item small" href="preview.php?id=' + id + '" id="preview"><i class="fa fa-eye text-info" aria-hidden="true"></i> Preview</a>' : '<a class="dropdown-item small disabled" href="#" id="receive"><i class="fa fa-lock text-secondary" aria-hidden="true"></i> <span class="ml-2">Restricted</span></a>')
                + '</div>').appendTo('body');
        ...
        $(document).on('contextmenu', '#ticket-table tbody tr', function (e) {
            ...
            var menu = $('<div class="dropdown-menu small removedrop" style="display:block; position:absolute; z-index:1000;">'
                + (branchid == '<?php echo $_SESSION['branchid']; ?>' ? '<a class="dropdown-item small" href="preview.php?id=' + id + '" id="preview"><i class="fa fa-eye text-info" aria-hidden="true"></i> Preview</a>' : '<a class="dropdown-item small disabled" href="#" id="receive"><i class="fa fa-lock text-secondary" aria-hidden="true"></i> <span class="ml-2">Restricted</span></a>')
                // + (branchid == '<?php echo $_SESSION['branchid']; ?>' ? '<a class="dropdown-item small" href="edit.php?id=' + id + '" id="edit"><i class="fa fa-edit text-primary" aria-hidden="true"></i> Edit</a>' : '')
                + '</div>').appendTo('body');
            menu.css({ top: e.pageY + 'px', left: e.pageX + 'px' });
